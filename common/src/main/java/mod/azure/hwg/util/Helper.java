...
public class Helper {
    ...
    public static void removeAmmo(Item ammo, Player playerEntity) {
        if (
            playerEntity.getItemInHand(playerEntity.getUsedItemHand()).getItem() instanceof AzureAnimatedGunItem
    ...
    private static boolean checkDistance(BlockPos blockPosA, BlockPos blockPosB) {
        return Math.abs(blockPosA.getX() - blockPosB.getX()) <= 2 && Math.abs(blockPosA.getY() - blockPosB.getY()) <= 2
            && Math.abs(blockPosA.getZ() - blockPosB.getZ()) <= 2;
    }
    ...
    public static SBulletEntity createSilverBullet(Level level, LivingEntity shooter) {
        var bulletEntity = new SBulletEntity(
            level,
            new ItemStack(Items.AIR),
            shooter,
            shooter.getX(),
