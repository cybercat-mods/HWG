    ...
    public HWGMobRender(EntityRendererProvider.Context renderManager, EntityEnum entityType) {
        ...
        addRenderLayer(new BlockAndItemGeoLayer<>(this) {
            ...
            protected void renderStackForBone(PoseStack poseStack, GeoBone bone, ItemStack stack, T animatable, MultiBufferSource bufferSource, float partialTick, int packedLight, int packedOverlay) {
                if (animatable.getMainHandItem().is(HWGItems.MINIGUN.get())) {
                ...
                if (animatable.getMainHandItem().is(HWGItems.MINIGUN.get())) {
                    ...
                    poseStack.mulPose(Axis.YP.rotationDegrees(animatable.getMainHandItem().is(HWGItems.BRIMSTONE.get()) ? 0 : 16));
                } else {
                    poseStack.mulPose(Axis.YP.rotationDegrees(0));
                }
                poseStack.mulPose(Axis.ZP.rotationDegrees(0f));
                if (animatable instanceof SpyEntity || animatable instanceof MercEntity) {
                    if (animatable.getMainHandItem().is(HWGItems.MINIGUN.get())) poseStack.translate(0.0D, 0.15D, -0.1D);
                    else poseStack.translate(0.0D, 0.1D, -0.1D);
                } else if (animatable instanceof TechnodemonEntity) {
                    if (animatable.getMainHandItem().is(HWGItems.BRIMSTONE.get())) poseStack.translate(0.0D, 0.15D, -0.7D);
                    else poseStack.translate(0.2D, 0.15D, -0.65D);
                } else if (animatable instanceof TechnodemonGreaterEntity) {
                    if (animatable.getMainHandItem().is(HWGItems.MINIGUN.get())) poseStack.translate(0.1D, 0.15D, -0.5D);
                    else poseStack.translate(0.1D, 0.1D, -0.1D);
                    if (animatable.getMainHandItem().is(HWGItems.BRIMSTONE.get())) poseStack.scale(1.1F, 1.1F, 1.1F);
                }
