...
public class SpyEntity extends HWGEntity implements SmartBrainOwner<SpyEntity> {
	...
	public SpyEntity(EntityType<SpyEntity> entityType, Level worldIn) {
		...
		xpReward = HWGMod.config.spy_exp;
	}
	...
	public static AttributeSupplier.Builder createMobAttributes() {
		return LivingEntity.createLivingAttributes().add(Attributes.FOLLOW_RANGE, 25.0D).add(Attributes.MOVEMENT_SPEED, 0.35D).add(Attributes.MAX_HEALTH, HWGMod.config.spy_health).add(Attributes.ARMOR, 3).add(Attributes.ARMOR_TOUGHNESS, 1D).add(Attributes.ATTACK_DAMAGE, 10D).add(Attributes.ATTACK_KNOCKBACK, 1.0D);
	}
