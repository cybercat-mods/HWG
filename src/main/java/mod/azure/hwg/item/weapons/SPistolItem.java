				...
				if (!worldIn.isClientSide) {
					...
					if (result != null) {
						if (result.getEntity()instanceof LivingEntity livingEntity)
							livingEntity.hurt(DamageSource.playerAttack(playerentity), HWGMod.config.silenced_pistol_damage);
					} else {
					...
					triggerAnim(playerentity, GeoItem.getOrAssignId(stack, (ServerLevel) worldIn), "shoot_controller", "firing2");
				}
