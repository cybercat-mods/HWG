...
public class Assasult1Item extends AnimatedItem {
    ...
    public Assasult1Item(String animation) {
        super(new Item.Properties().stacksTo(1).durability(HWGMod.config.gunconfigs.smgconfigs.smg_cap + 1));
                ...
                if (!worldIn.isClientSide) {
                    ...
                    if (result != null) {
                        if (result.getEntity() instanceof LivingEntity livingEntity) {
                            ...
                            livingEntity.hurt(playerentity.damageSources().playerAttack(playerentity), HWGMod.config.gunconfigs.smgconfigs.smg_damage);
                    ...
                    worldIn.playSound(null, playerentity.getX(), playerentity.getY(), playerentity.getZ(), HWGSounds.SMG, SoundSource.PLAYERS, 0.25F, 1.0F / (worldIn.random.nextFloat() * 0.4F + 1.2F) + 0.5F);
    ...
    public void reload(Player user, InteractionHand hand) {
        if (user.getItemInHand(hand).getItem() instanceof Assasult1Item) {
            while (!user.isCreative() && user.getItemInHand(hand).getDamageValue() != 0 && user.getInventory().countItem(HWGItems.BULLETS) > 0) {
                ...
                user.getCooldowns().addCooldown(this, HWGMod.config.gunconfigs.smgconfigs.smgReloadCooldown);
                ...
                if (!user.level().isClientSide)
                    triggerAnim(user, GeoItem.getOrAssignId(user.getItemInHand(hand), (ServerLevel) user.level()), "shoot_controller", "smgreload");
            }
        }
    }
