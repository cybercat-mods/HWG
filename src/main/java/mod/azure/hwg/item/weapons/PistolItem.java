...
public class PistolItem extends AnimatedItem {
				...
				if (!worldIn.isClientSide) {
					...
					if (result != null) {
						if (result.getEntity()instanceof LivingEntity livingEntity)
							livingEntity.hurt(playerentity.damageSources().playerAttack(playerentity), HWGConfig.pistol_damage);
					} else {
					...
					worldIn.playSound((Player) null, playerentity.getX(), playerentity.getY(), playerentity.getZ(), HWGSounds.PISTOL, SoundSource.PLAYERS, 0.5F, 1.0F / (worldIn.random.nextFloat() * 0.4F + 1.2F) + 1F * 0.5F);
	...
	public void inventoryTick(ItemStack stack, Level world, Entity entity, int slot, boolean selected) {
		if (world.isClientSide)
			if (((Player) entity).getMainHandItem().getItem() instanceof PistolItem) {
	...
	public void reload(Player user, InteractionHand hand) {
		if (user.getItemInHand(hand).getItem() instanceof PistolItem) {
	...
	public BulletEntity createArrow(Level worldIn, ItemStack stack, LivingEntity shooter) {
		var bullet = new BulletEntity(worldIn, shooter, HWGConfig.pistol_damage);
		return bullet;
	}
