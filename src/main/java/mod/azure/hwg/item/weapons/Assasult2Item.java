...
public class Assasult2Item extends AnimatedItem {
    ...
    public Assasult2Item(String animation) {
        super(new Item.Properties().stacksTo(1).durability(HWGMod.config.gunconfigs.tommyconfigs.tommy_cap + 1));
    ...
    public void onUseTick(Level worldIn, LivingEntity entityLiving, ItemStack stack, int count) {
        if (entityLiving instanceof Player playerentity && stack.getDamageValue() < (stack.getMaxDamage() - 1) && !playerentity.getCooldowns().isOnCooldown(this)) {
            playerentity.getCooldowns().addCooldown(this, HWGMod.config.gunconfigs.tommyconfigs.tommy_cooldown);
            if (!worldIn.isClientSide) {
                ...
                if (result != null) {
                    if (result.getEntity() instanceof LivingEntity livingEntity) {
                        ...
                        livingEntity.hurt(playerentity.damageSources().playerAttack(playerentity), HWGMod.config.gunconfigs.tommyconfigs.tommy_damage);
                ...
                worldIn.playSound(null, playerentity.getX(), playerentity.getY(), playerentity.getZ(), HWGSounds.TOMMY, SoundSource.PLAYERS, 0.25F, 1.0F / (worldIn.random.nextFloat() * 0.4F + 1.2F) + 0.5F);
    ...
    public void inventoryTick(ItemStack stack, Level world, Entity entity, int slot, boolean selected) {
        if (world.isClientSide && entity instanceof Player player && player.getMainHandItem().getItem() instanceof Assasult2Item)
    ...
    public void reload(Player user, InteractionHand hand) {
        if (user.getItemInHand(hand).getItem() instanceof Assasult2Item) {
            while (!user.isCreative() && user.getItemInHand(hand).getDamageValue() != 0 && user.getInventory().countItem(HWGItems.BULLETS) > 0) {
                ...
                user.getCooldowns().addCooldown(this, HWGMod.config.gunconfigs.tommyconfigs.tommyReloadCooldown);
                ...
                if (!user.level().isClientSide) {
                    if (user.getRandom().nextInt(0, 100) >= 95)
                        triggerAnim(user, GeoItem.getOrAssignId(user.getItemInHand(hand), (ServerLevel) user.level()), "shoot_controller", "tommyreload2");
                    else
                        triggerAnim(user, GeoItem.getOrAssignId(user.getItemInHand(hand), (ServerLevel) user.level()), "shoot_controller", "tommyreload");
                }
            }
        }
    }
    ...
    public BulletEntity createArrow(Level worldIn, ItemStack stack, LivingEntity shooter) {
        return new BulletEntity(worldIn, shooter, HWGMod.config.gunconfigs.tommyconfigs.tommy_damage);
    }
