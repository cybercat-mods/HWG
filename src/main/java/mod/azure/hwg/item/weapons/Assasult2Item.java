...
public class Assasult2Item extends AnimatedItem {
	...
	public void onAnimationSync(int id, int state) {
		if (state == ANIM_OPEN) {
			...
			if (controller.getAnimationState() == AnimationState.Stopped) {
				...
				controller.setAnimation(new AnimationBuilder().addAnimation(this.animation, EDefaultLoopTypes.PLAY_ONCE));
			}
		}
	}
	...
	public void inventoryTick(ItemStack stack, World world, Entity entity, int slot, boolean selected) {
		if (world.isClient) {
			if (((PlayerEntity) entity).getMainHandStack().getItem() instanceof Assasult2Item) {
