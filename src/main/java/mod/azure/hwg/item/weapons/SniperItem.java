...
public class SniperItem extends AnimatedItem {
			...
			if (stack.getDamageValue() < (stack.getMaxDamage() - 1)) {
				...
				if (!worldIn.isClientSide) {
					...
					abstractarrowentity.shootFromRotation(playerentity, playerentity.getXRot(), playerentity.getYRot(), 0.0F,
							1.0F * 3.0F, 1.0F);
					abstractarrowentity.tickCount = -15;

					stack.hurtAndBreak(1, entityLiving, p -> p.broadcastBreakEvent(entityLiving.getUsedItemHand()));
					worldIn.addFreshEntity(abstractarrowentity);
					worldIn.playSound((Player) null, playerentity.getX(), playerentity.getY(),
							playerentity.getZ(), HWGSounds.SNIPER, SoundSource.PLAYERS, 0.25F,
	...
	public void reload(Player user, InteractionHand hand) {
		if (user.getItemInHand(hand).getItem() instanceof SniperItem) {
			while (!user.isCreative() && user.getItemInHand(hand).getDamageValue() != 0
					&& user.getInventory().countItem(HWGItems.SNIPER_ROUND) > 0) {
				...
				user.getCommandSenderWorld().playSound((Player) null, user.getX(), user.getY(), user.getZ(),
						HWGSounds.SNIPERRELOAD, SoundSource.PLAYERS, 0.5F, 1.0F);
			}
		}
	}
	...
	public BulletEntity createArrow(Level worldIn, ItemStack stack, LivingEntity shooter) {
		BulletEntity arrowentity = new BulletEntity(worldIn, shooter,
				shooter.isShiftKeyDown() ? HWGConfig.sniper_scoped_damage : HWGConfig.sniper_damage);
		return arrowentity;
	}
