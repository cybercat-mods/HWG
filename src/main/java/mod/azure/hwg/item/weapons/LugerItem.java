...
public class LugerItem extends AnimatedItem {
			...
			if (stack.getDamage() < (stack.getMaxDamage() - 1)) {
				...
				if (!worldIn.isClient) {
					...
					worldIn.playSound((PlayerEntity) null, playerentity.getX(), playerentity.getY(),
							playerentity.getZ(), HWGSounds.LUGER, SoundCategory.PLAYERS, 0.5F,
	...
	public void onAnimationSync(int id, int state) {
		if (state == ANIM_OPEN) {
			...
			if (controller.getAnimationState() == AnimationState.Stopped) {
				...
				controller.setAnimation(new AnimationBuilder().addAnimation("luger", EDefaultLoopTypes.PLAY_ONCE));
			}
		}
	}
	...
	public BulletEntity createArrow(World worldIn, ItemStack stack, LivingEntity shooter) {
		BulletEntity arrowentity = new BulletEntity(worldIn, shooter, HWGConfig.luger_damage);
		return arrowentity;
	}
