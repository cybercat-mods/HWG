...
public class AssasultItem extends AnimatedItem {
				...
				if (!worldIn.isClientSide) {
					...
					if (result != null) {
						if (result.getEntity()instanceof LivingEntity livingEntity) {
							livingEntity.hurt(DamageSource.playerAttack(playerentity), HWGMod.config.ak47_damage);
	...
	public void inventoryTick(ItemStack stack, Level world, Entity entity, int slot, boolean selected) {
		if (world.isClientSide)
			if (((Player) entity).getMainHandItem().getItem() instanceof AssasultItem) {
		...
		if (user.getItemInHand(hand).getItem() instanceof AssasultItem) {
			while (!user.isCreative() && user.getItemInHand(hand).getDamageValue() != 0 && user.getInventory().countItem(HWGItems.BULLETS) > 0) {
				...
				if (!user.getLevel().isClientSide)
					triggerAnim(user, GeoItem.getOrAssignId(user.getItemInHand(hand), (ServerLevel) user.getCommandSenderWorld()), "shoot_controller", "akreload");
			}
		}
	}
